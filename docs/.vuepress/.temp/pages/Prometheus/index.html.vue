...
<p>Prometheus 是一个开源系统监控和警报工具包，Prometheus 受启发于 Google 的Brogmon 监控系统（相似的 Kubernetes 是从 Google的 Brog 系统演变而来），从 2012 年开始由前 Google 工程师在 Soundcloud 以开源软件的形式进行研发，并且于 2015 年早期对外发布早期版本。</p>
<p>2016 年 5 月继 Kubernetes 之后成为第二个正式加入 CNCF 基金会的项目，同年 6 月正式发布 1.0 版本。2017 年底发布了基于全新存储层的 2.0 版本，能更好地与容器平台、云平台配合。</p>
...
<p><img src="@source/Prometheus/img/e9c8888b296ffc141fc65c499afed961.png" alt="prometheus architecture"></p>
...
</code></pre></div><h4 id="_1-1-样本" tabindex="-1"><a class="header-anchor" href="#_1-1-样本" aria-hidden="true">#</a> 1.1 样本</h4>
...
</code></pre></div><h4 id="_1-2-指标-metric" tabindex="-1"><a class="header-anchor" href="#_1-2-指标-metric" aria-hidden="true">#</a> 1.2 指标(Metric)</h4>
...
</code></pre></div><h3 id="_2、指标-metric-的4种类型" tabindex="-1"><a class="header-anchor" href="#_2、指标-metric-的4种类型" aria-hidden="true">#</a> 2、指标(Metric)的4种类型</h3>
...
</code></pre></div><h4 id="_2-1-counter-只增不减的计数器" tabindex="-1"><a class="header-anchor" href="#_2-1-counter-只增不减的计数器" aria-hidden="true">#</a> 2.1 Counter：只增不减的计数器</h4>
...
</code></pre></div><p>与Summary类型的指标相似之处在于Histogram类型的样本同样会反应当前指标的记录的总数（以_count作为后缀）以及其值的总量（以_sum作为后缀）。不同在于Histogram指标直接反应了在不同区间内样本的个数，区间通过标签len进行定义。</p>
...
<p>需要特别注意的是,假设采样数据 metric 叫做 x(指标名)，如果 × 是 histogram 或 summary 类型必需满足以下条件：</p>
<ul>
<li>采样数据的总和应表示为 x_sum 。</li>
<li>采样数据的总量应表示为 x_countl 。</li>
<li>summary 类型的采样数据的quantile应表示为 x{quantile=&quot;y&quot;}。</li>
...
<h3 id="_3、job-任务-和instances-实例" tabindex="-1"><a class="header-anchor" href="#_3、job-任务-和instances-实例" aria-hidden="true">#</a> 3、job（任务）和instances（实例）</h3>
<h4 id="_3-1-概述" tabindex="-1"><a class="header-anchor" href="#_3-1-概述" aria-hidden="true">#</a> 3.1 概述</h4>
<p>在Prometheus中，任何被采集的目标，即每一个暴露监控样本数据的HTTP服务都称为一个实例（Instance），例如在当前主机上运行的node exporter可以被称为一个实例(Instance)。而具有相同采集目的的实例集合称为任务（Job）。</p>
<h4 id="_3-2-job-任务" tabindex="-1"><a class="header-anchor" href="#_3-2-job-任务" aria-hidden="true">#</a> 3.2 job（任务）</h4>
<p>例如，以下2个复制实例的node作业：</p>
<div class="language-bash" data-ext="sh"><pre v-pre class="language-bash"><code>* job: <span class="token function">node</span>
...
</code></pre></div><h4 id="_3-3-instances-实例" tabindex="-1"><a class="header-anchor" href="#_3-3-instances-实例" aria-hidden="true">#</a> 3.3 instances（实例）</h4>
<p>通过在prometheus.yml配置文件中，添加如下配置。我们让Prometheus可以从node exporter暴露的服务中获取监控指标数据。</p>
<div class="language-bash" data-ext="sh"><pre v-pre class="language-bash"><code>scrape_configs:
...
</code></pre></div><p>当我们需要采集不同的监控指标（例如：主机、MySQL、Nginx）时，我们只需要运行相应的监控采集程序，在Prometheus Server配置这些Exporter实例的访问地址。</p>
<h4 id="_3-4-实例的状态" tabindex="-1"><a class="header-anchor" href="#_3-4-实例的状态" aria-hidden="true">#</a> 3.4 实例的状态</h4>
<p>除了通过使用“up”表达式查询当前所有Instance的状态以外，还可以通过Prometheus UI中的<a href="http://10.0.0.112:9090/targets" target="_blank" rel="noopener noreferrer">Targets<ExternalLinkIcon/></a>页面查看当前所有的监控采集任务，以及各个任务下所有实例的状态。</p>
<h2 id="七、exporter" tabindex="-1"><a class="header-anchor" href="#七、exporter" aria-hidden="true">#</a> 七、Exporter</h2>
<h3 id="_1、概述-1" tabindex="-1"><a class="header-anchor" href="#_1、概述-1" aria-hidden="true">#</a> 1、概述</h3>
<p>所有可以向Prometheus提供监控样本数据的程序都可以被称为一个Exporter。而Exporter的一个实例称为target，如下所示，Prometheus通过轮询的方式定期从这些target中获取样本数据。</p>
<p>注：安装好Exporter后会暴露一个 <code v-pre>http://ip:端口/metrics</code> 的HTTP服务，通过Prometheus添加配置 <code v-pre>- targets: ['node_exporter:9100']</code> （默认会加上/metrics），Prometheus就可以采集到这个 <code v-pre>http://ip:端口/metrics</code> 里面所有监控样本数据</p>
<h3 id="_2、exporter的来源" tabindex="-1"><a class="header-anchor" href="#_2、exporter的来源" aria-hidden="true">#</a> 2、Exporter的来源</h3>
<p>从Exporter的来源上来讲，主要分为两类：</p>
<h4 id="_2-1-社区提供的" tabindex="-1"><a class="header-anchor" href="#_2-1-社区提供的" aria-hidden="true">#</a> 2.1 社区提供的</h4>
<ul>
<li>社区提供的（https://prometheus.io/docs/instrumenting/exporters/）</li>
</ul>
...
<h4 id="_2-2-用户自定义的" tabindex="-1"><a class="header-anchor" href="#_2-2-用户自定义的" aria-hidden="true">#</a> 2.2 用户自定义的</h4>
...
<h3 id="_3、exporter-类型" tabindex="-1"><a class="header-anchor" href="#_3、exporter-类型" aria-hidden="true">#</a> 3、Exporter 类型</h3>
<p>通常来说可以将要 Exporter 分为两类：</p>
<ul>
<li>直接采集型。</li>
</ul>
<p>这类 Exporter 直接内置了相应的应用程序，用于向 Prometheus 直接提供 target 数据支持。这样设计的好处是，可以更好地监控各自系统的内部运行状态，同时也适合更多自定义监控指标的项目实施。如 cAdvisor、Kubernetes 等，它们均内置了用于向 Prometheus 提供监控数据的端点。</p>
<ul>
<li>间接采集型</li>
</ul>
<p>原始监控目标并不直接支持 Prometheus，需要我们使用 Prometheus 提供的 Client Libary 编写该监控目标的监控采集程序，用户可以将该程序独立运行，去获取指定的各类监控数据监控值。比如 Node exporter，还有 Nginx exporter，以及数据库或网站 HTTP 应用类的 Expoter 等</p>
<h3 id="_4、exporter规范" tabindex="-1"><a class="header-anchor" href="#_4、exporter规范" aria-hidden="true">#</a> 4、Exporter规范</h3>
<p>所有的Exporter程序都需要按照Prometheus的规范，返回监控的样本数据。以Node Exporter为例，当访问http://10.0.0.112:9100/metrics地址时会返回以下内容：</p>
<div class="language-bash" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token comment"># HELP go_gc_duration_seconds A summary of the pause duration of garbage collection cycles.</span>
...
</code></pre></div><p>以#开始的行通常都是注释内容。这些样本数据集合说明如下：</p>
<ul>
<li>以#HELP开始的行，表示metric的帮助与说明注释，可以包含当前监控指标名称和对应的说明信息。</li>
<li>以#TYPE开始的行，表示定义metric类型，可以包含当前监控指标名称和类型，类型有Counter、Gauge、Histogram、Summary和Untyped。</li>
<li>非#开头的行，就是监控样本数据</li>
</ul>
<h4 id="_4-1-监控样本数据规范" tabindex="-1"><a class="header-anchor" href="#_4-1-监控样本数据规范" aria-hidden="true">#</a> 4.1 监控样本数据规范</h4>
<div class="language-bash" data-ext="sh"><pre v-pre class="language-bash"><code>metric_name <span class="token punctuation">[</span> <span class="token string">"{"</span> label_name <span class="token string">"="</span> <span class="token variable"><span class="token variable">`</span>"<span class="token variable">`</span></span> label_value <span class="token variable"><span class="token variable">`</span>"<span class="token variable">`</span></span> <span class="token punctuation">{</span> <span class="token string">","</span> label_name <span class="token string">"="</span> <span class="token variable"><span class="token variable">`</span>"<span class="token variable">`</span></span> label_value <span class="token variable"><span class="token variable">`</span>"<span class="token variable">`</span></span> <span class="token punctuation">}</span> <span class="token punctuation">[</span> <span class="token string">","</span> <span class="token punctuation">]</span> <span class="token string">"}"</span> <span class="token punctuation">]</span> value <span class="token punctuation">[</span> timestamp <span class="token punctuation">]</span>
</code></pre></div><p>其中metric_name和label_name必须遵循PromQL的格式规范要求。value是一个float格式的数据，timestamp的类型为int64（从1970-01-01 00:00:00以来的毫秒数），timestamp为可选默认为当前时间。具有相同metric_name的样本必须按照一个组的形式排列，并且每一行必须是唯一的指标名称和标签键值对组合。</p>
<div class="language-bash" data-ext="sh"><pre v-pre class="language-bash"><code>go_memstats_mcache_sys_bytes<span class="token punctuation">{</span><span class="token punctuation">}</span> <span class="token number">15600</span>
