...
<p><img src="@source/kube/img/image-20230728155640161.png" alt="adsf"></p>
...
<p><img src="@source/kube/img/container_evolution.svg" alt="adfadd"></p>
...
<p><img src="@source/kube/img/v2-fcdfe793f83067297bc1b33bc0d2836c_1440w.jpg" alt="k8s"></p>
...
<p><img src="@source/kube/img/k8s-architecture.png" alt="11-K8s架构及基本概念 - 图1"></p>
<ul>
<li><strong>总结</strong>
<ul>
...
<p><img src="@source/kube/img/image-20230728172728017.png" alt="image-20230728172728017"></p>
...
<p><img src="@source/kube/img/1156961-20181220140922339-685966095.png" alt="img"></p>
...
<li>**运行多个协同工作的容器的 Pod。**Pod 可能封装由多个紧密耦合且需要共享资源的共处容器组成的应用程序。这些位于同一位置的容器可能形成单个内聚的服务单元 一一 一个容器将文件从共享卷提供给公众，而另一个单独的 ”边车“ (sidecar) 容器则刷新或更新这些文件。Pod 将这些容器和存储资源打包为一个可管理的实体。</li>
</ul>
...
<span class="token key atrule">spec</span><span class="token punctuation">:</span>
  <span class="token key atrule">containers</span><span class="token punctuation">:</span>
    <span class="token punctuation">-</span> <span class="token key atrule">name</span><span class="token punctuation">:</span> redis
      ...
      <span class="token key atrule">imagePullPolicy</span><span class="token punctuation">:</span> IfNotPresent
  <span class="token key atrule">restartPolicy</span><span class="token punctuation">:</span> Always
</code></pre></div><h4 id="_6-5-容器深针" tabindex="-1"><a class="header-anchor" href="#_6-5-容器深针" aria-hidden="true">#</a> 6.5 容器深针</h4>
...
<li><code v-pre>readinessProbe</code> <strong>指示容器是否准备好为请求提供服</strong>。如果就绪态探测失败，端点控制器将从与 Pod 匹配的所有服务的端点列表中删除该 Pod 的 IP 地址。初始延迟之前的就绪态的状态值默认为 <code v-pre>Failure</code>。如果容器不提供就绪态探针，则默认状态为 <code v-pre>Success</code>。</li>
...
<li>
<p>CPU 资源限制：为容器设置 CPU **request（请求）**和 CPU <strong>limit（限制）</strong>。容器使用的 CPU 不能超过所配置的限制。如果系统有空闲的 CPU 时间，则可以保证给容器分配其所请求数量的 CPU 资源。</p>
<blockquote>
...
<p>**解决方法：**增加这个参数 <code v-pre>spec.template.spec.hostNetwork: true</code></p>
</blockquote>
...
<p><code v-pre>Kubernetes 通常不会直接创建 Pod，而是通过 Controller 来管理 Pod 的。</code>**Controller中定义了 Pod 的部署特性，比如有几个副本、在什么样的 Node 上运行等。**通俗的说可以认为 Controller 就是用来管理 Pod 一个对象。其核心作用可以通过一句话总结：<code v-pre>通过监控集群的公共状态，并致力于将当前状态转变为期望的状态。</code></p>
...
<p><img src="@source/kube/img/image-20230730212203875.png" alt="image-20230730212203875"></p>
