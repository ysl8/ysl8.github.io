...
</code></pre></div><p>访问地址</p><table><thead><tr><th>应用</th><th>访问地址</th><th>备注</th></tr></thead><tbody><tr><td>prometheus</td><td>http://10.0.0.112:9090</td><td>无用户和密码</td></tr><tr><td>监控指标</td><td>http://10.0.0.112:9090/metrics</td><td></td></tr></tbody></table><h4 id="_2-2-安装alertmanager" tabindex="-1"><a class="header-anchor" href="#_2-2-安装alertmanager" aria-hidden="true">#</a> 2.2 安装alertmanager</h4><p>下载alertmanager</p>`,22),m={href:"https://prometheus.io/download/",target:"_blank",rel:"noopener noreferrer"},g=n(`<ul><li>注：在root用户下执行</li></ul><div class="language-bash" data-ext="sh"><pre class="language-bash"><code><span class="token comment"># 下载alertmanager二进制压缩包</span>
...
</code></pre></div><p>1.1 样本</p><p>Prometheus 会将所有采集到的监控样本数据以时间序列的方式保存在 <code>内存数据库</code>中，并且定时保存到硬盘上。时间序列是按照时间戳和值的序列顺序存放的，我们称之为向量(vector)，每条时间序列通过指标名称(metrics name)和一组标签集(label)命名。如下所示，可以将时间序列理解为一个以时间为X轴的数字矩阵：</p><div class="language-bash" data-ext="sh"><pre class="language-bash"><code>∧
...
</code></pre></div><p>指标(Metric)</p><p>在形式上，所有的指标（Matric)都通过如下格式标示：</p><div class="language-bash" data-ext="sh"><pre class="language-bash"><code><span class="token operator">&lt;</span>metric name<span class="token operator">&gt;</span><span class="token punctuation">{</span><span class="token operator">&lt;</span>label name<span class="token operator">&gt;=</span><span class="token operator">&lt;</span>label value<span class="token operator">&gt;</span>, <span class="token punctuation">..</span>.<span class="token punctuation">}</span>
...
</code></pre></div><p>与Summary类型的指标相似之处在于Histogram类型的样本同样会反应当前指标的记录的总数（以_count作为后缀）以</p><div class="language-bash" data-ext="sh"><pre class="language-bash"><code>及其值的总量（以
...
</code></pre></div>`,185);function k(b,_){const e=p("ExternalLinkIcon");return r(),c("div",null,[d,a("p",null,[a("a",i,[s("官网下载地址"),t(e)]),s("，获取最新下载URL")]),h,a("p",null,[a("a",m,[s("官网下载地址"),t(e)]),s("，获取最新下载URL")]),g])}const f=o(u,[["render",k],["__file","start.html.vue"]]);export{f as default};
